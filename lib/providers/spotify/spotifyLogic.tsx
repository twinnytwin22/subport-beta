...
export const handleSpotifyAction = async (
  ...
  try {
    ...
    if (!accessToken) {
      ...
      const authOptions = {
        method: spotifyActions({ spotifyId }).find((a) => a.action === action)
          ?.method,
        headers: createSpotifyHeaders(accessToken)
      };

      let endpoint = `https://api.spotify.com/v1${spotifyActions({
        spotifyId
      }).find((a) => a.action === action)?.endpoint}`;
      ...
      if (response.ok) {
        const toastMessage = spotifyActions({}).find(
          (a) => a?.action === action
        )?.toast?.success;
        if (toastMessage) {
        ...
        const errorMessage = spotifyActions({}).find((a) => a.action === action)
          ?.toast?.error;
...
export default async function refreshSpotifyToken(token: any) {
  const basicAuth = Buffer.from(`${spotifyClientId}:${spotifySecret}`).toString(
    'base64'
  );
  ...
  try {
    const response = await fetch(
      SPOTIFY_REFRESH_TOKEN_URL + requestBody.toString(),
      {
        method: 'POST',
        headers: {
