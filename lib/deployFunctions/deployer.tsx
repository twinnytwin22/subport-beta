...
export const deployCollectible = async (collectibleData: any) => {
  ...
  try {
    ...
    try {
      ...
      const metaDataHash: string | undefined = await new Promise(
        (resolve, reject) => {
          setTimeout(async () => {
            try {
      ...
      const tokenDataHash: string | undefined = await new Promise(
        (resolve, reject) => {
          setTimeout(async () => {
            try {
      ...
      if (metaDataHash && tokenDataHash) {
        ...
        const getSlug =
          collectibleData?.artist_name + '-' + collectibleData?.name;
        let slug: string | undefined = getSlug
          .toLowerCase()
          .replace(/[^a-zA-Z0-9-]+/g, '-');

        // Check if slug already exists in Supabase
        const { data: existingDrops, error: existingDropsError } =
          await supabase.from('drops').select().eq('slug', slug);
