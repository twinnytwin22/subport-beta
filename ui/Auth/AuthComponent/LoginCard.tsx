    ...
    <div className="relative z-[9999999px]">
      {!isHomePage ? (
        <div className="flex flex-col px-6 py-4 border-b rounded-t dark:border-zinc-600 relative min-w-full w-sm lg:w-full">
          <div
            className="cursor-pointer p-2 border dark:border-zinc-700 border-zinc-300 rounded text-sm w-24 text-center text-black dark:text-white bg-white dark:bg-black absolute right-5 top-3"
            onClick={() => close(false)}
          >
            Close
          </div>
      ...
      <div className="p-6 w-full my-4">
        {!isHomePage ? (
          ...
          <h1 className="text-2xl pb-4 font-semibold text-zinc-600 dark:text-zinc-100">
            Connect with Spotify to login.
          </h1>
        )}
        <ul className="my-4 space-y-3">
          ...
          >
            <div
              className={
                'flex items-center p-3 text-base font-bold text-zinc-900 rounded-md bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 dark:text-white'
              }
            >
              <img className="w-5 h-5" src="/images/icons/spotify.png" />
              <span className="flex-1 ml-3 whitespace-nowrap">Spotify</span>
            </div>
          </li>
          <li
