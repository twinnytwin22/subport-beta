...
export const CommentModal = ({ dropId, close }: any) => {
  ...
  return (
    <div className="absolute top-0 left-0 right-0 bottom-0 justify-center items-center z-50 isolate">
      <div className="bg-white dark:bg-black opacity-80 h-full w-full z-10 absolute"></div>{' '}
      {/* Updated z-index here */}
      <p
        className="cursor-pointer absolute p-2 border dark:border-zinc-700 border-zinc-300 rounded text-sm z-20 left-5 top-5 text-black dark:text-white bg-white dark:bg-black"
        onClick={close}
      >
        Close
      </p>{' '}
      {/* Updated z-index here */}
      {profile && (
        <div className="w-full h-4 rounded max-w-md mx-auto relative z-20 items-center mt-20 text-black">
          {' '}
          {/* Updated z-index here */}
          ...
          <div className="flex space-x-2 items-center content-end justify-end text-white">
            ...
            <button
              className="text-xs p-1.5 bg-red-600 hover:bg-red-500 rounded-md  text-center font-bold"
      ...
      <div className="relative z-10 mt-28 h-full overflow-y-scroll max-h-96 mx-2.5">
        {' '}
        {/* Lower z-index here */}
        ...
        {[...comments]?.reverse()?.map((comment: any, i) => (
          <div
            key={i}
            className="p-2.5 rounded-md text-sm bg-zinc-100 dark:bg-zinc-950 my-2 border-zinc-300 dark:border-zinc-900 border relative"
          >
            <Link
            ...
            <div className="border-l-2 m-2 h-fit border-blue-600">
              <p className="pl-2 text-sm">{comment?.comment}</p>
            </div>
            {userId && userId === comment.user_id ? (
              <div
                className="text-sm text-blue-600 underline cursor-pointer absolute right-3 bottom-3 hover:scale-125 duration-300 ease-in-out"
