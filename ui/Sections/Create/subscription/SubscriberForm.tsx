...
function SubscriberForm({ close, sub, isAuthedUser }: any) {
  ...
  const handleSubscriptionBundleChange = (
  ...
  const getTotalPrice = () => {
    if (selectedBundle) {
      const tier = subscriptionTiers.find(
        (tier) => tier.months === parseInt(selectedBundle)
      );
      if (tier) {
  ...
  return (
    <div className="bg-white dark:bg-black p-8 rounded-md shadow-lg border-zinc-200 dark:border-zinc-700 border max-w-md  w-96 scale-90 md:scale-100 z-[9999] isolate mx-auto subscribe-modal">
      <p
        className=" cursor-pointer absolute p-2 border dark:border-zinc-800 border-zinc-200 rounded text-sm"
        onClick={close}
      >
        Close
      </p>
      <h1 className="text-center text-xl text-black dark:text-white">
        {isAuthedUser ? 'Your Subscription' : 'Subscribe Now'}
      </h1>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-4">
        <h3 className="mb-2 font-bold text-zinc-900 dark:text-white">
          {isAuthedUser ? 'Accepted payments' : 'Pay via'}
        </h3>
        ...
        <div className="space-y-2">
          ...
          <ul className="space-y-2">
            ...
            {subscriptionTiers
              .filter((tier) => tier.active) // Filter out inactive tiers
              .map((tier) => (
        ...
        {!isAuthedUser ? (
          <div className="flex items-center space-x-2">
            ...
            <div className="flex items-center space-x-2">
              <label className="font-bold" htmlFor="totalPrice">
                Total Price:
              </label>
              <p>${getTotalPrice()}</p>
            </div>
