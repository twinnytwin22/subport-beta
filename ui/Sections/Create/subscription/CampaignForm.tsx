...
function CampaignForm({ close, price }: any) {
  ...
  return (
    <form className="p-8 space-y-2 text-base" onSubmit={handleSubmit(onSubmit)}>
      <h2 className="font-semibold text-center mb-4 text-xl md:text-2xl">
        ...
        <li className="w-full border-b border-zinc-200 sm:border-b-0 sm:border-r dark:border-zinc-600">
          <div className="flex items-center pl-3">
            <input
              type="radio"
              value="new"
              {...register('subscriberType', { required: true })}
              className="mr-2"
            />
            <label className="w-full py-3 ml-2 text-sm font-medium text-zinc-900 dark:text-zinc-300">
              New
            </label>
          </div>
        </li>
        ...
        <li className="w-full border-b border-zinc-200 sm:border-b-0 dark:border-zinc-600">
          <div className="flex items-center pl-3">
            <input
              ...
              className="mr-2"
            />
            <label className="w-full py-3 mr-2 text-sm font-medium text-zinc-900 dark:text-zinc-300">
      ...
      <div className="">
        {watch('promotionType') === 'freeTrial' && (
          <>
            ...
            <p className="text-xs">
              User will not be subscribed for $6.99 automatically, only by
              choice.
            </p>
          </>
        )}
        {watch('promotionType') === 'firstMonthDiscount' && (
          <>
            <h3 className="font-bold mt-4 mb-2">
              First month discount details:
            </h3>
            <p>First month duration: 30 days</p>
            <p>
              Promotional subscription: ${roundedPrice} USD for first month.
            </p>
            <p className="text-xs">
              User will not be subscribed for $6.99 automatically, only by
              choice.
