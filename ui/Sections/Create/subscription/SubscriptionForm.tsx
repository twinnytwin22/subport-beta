...
function SubscriptionForm() {
  ...
  const subscriptionTiers = [
    {
      months: 3,
      price: (pricePerMonth * 3 * 0.8).toFixed(2),
      discount: '20% off'
    },
    {
      months: 6,
      price: (pricePerMonth * 6 * 0.75).toFixed(2),
      discount: '25% off'
    },
    {
      months: 12,
      price: (pricePerMonth * 12 * 0.65).toFixed(2),
      discount: '35% off'
    }
  ];
    ...
    <div className="max-w-md mx-auto space-y-8 mb-24 ">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <li className="w-full border-b border-zinc-200 sm:border-b-0 sm:border-r dark:border-zinc-600">
            <div className="flex items-center pl-3">
              <input
                ...
                id="crypto"
                value="crypto"
                className={`w-4 h-4 text-blue-600 bg-zinc-100 border-zinc-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-zinc-700 dark:focus:ring-offset-zinc-700 focus:ring-2 dark:bg-zinc-600 dark:border-zinc-700 ${
                  errors.paymentMethod ? 'border-red-500' : 'border-zinc-300'
                }`}
              />
              <label
                htmlFor="crypto"
                className="w-full py-3 ml-2 text-sm font-medium text-zinc-900 dark:text-zinc-300"
              >
                Crypto
              </label>
        ...
        <div className="space-y-2">
          <label className="font-bold" htmlFor="pricePerMonth">
            Price per month
          </label>
          <input
            {...register('pricePerMonth', {
              required: 'Price per month is required',
              min: { value: 4.99, message: 'Minimum price per month is $4.99' }
            })}
            type="number"
            id="pricePerMonth"
            step="0.01"
            className={`border ${
              errors.pricePerMonth
                ? 'border-red-500'
                : 'border-zinc-300 dark:border-zinc-700 dark:bg-zinc-900 bg-white'
      ...
      {modalOpen && (
        <CampaignModal setModalOpen={setModalOpen} price={pricePerMonth} />
      )}
      ...
      {successModalOpen && (
        <div className="fixed inset-0 flex flex-col items-center justify-center z-50">
          <div className="bg-white dark:bg-zinc-950 rounded-md p-8 place-items-center">
            ...
            <p className="mb-4">
              You can view your active subscription in your profile:
            </p>
