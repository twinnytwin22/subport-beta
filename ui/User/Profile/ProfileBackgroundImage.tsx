...
import { supabase, useBgImagePath } from 'lib/constants';
...
function ProfileBackgroundImage({
  ...
  const uploadBg: React.ChangeEventHandler<HTMLInputElement> = async (
    event
  ) => {
    try {
      setUploading(true);
      ...
      let bgPromise = supabase.storage
        .from('profile_backgrounds')
        .upload(filePath, file);
      toast.promise(bgPromise, {
