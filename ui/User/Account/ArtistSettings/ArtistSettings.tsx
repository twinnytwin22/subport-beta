  ...
  async function updateProfile({
    ...
    if (artistData && user) {
      try {
        ...
        if (
          typeof artist_name !== 'undefined' &&
          artist_name !== artistData?.artist_name
        ) {
          updates.artist_name = artist_name;
        }
        if (
          typeof apple_url !== 'undefined' &&
          apple_url !== artistData?.apple_url
        ) {
          updates.apple_url = apple_url;
        }
        if (
          typeof amazon_url !== 'undefined' &&
          amazon_url !== artistData?.amazon_url
        ) {
          updates.amazon_url = amazon_url;
        }
        if (
          typeof deezer_url !== 'undefined' &&
          deezer_url !== artistData?.deezer_url
        ) {
        ...
        if (
          typeof spotify_url !== 'undefined' &&
          spotify_url !== artistData?.spotify_url
        ) {
          updates.spotify_url = spotify_url;
        }
        if (
          typeof tidal_url !== 'undefined' &&
          tidal_url !== artistData?.tidal_url
        ) {
    ...
    user && (
      <>
        <div className="mx-auto w-full max-w-sm content-start items-center h-full my-8 flex-col justify-between mt-8">
          ...
          <div
            className={`py-8 w-full mx-auto flex justify-center ${!profile.is_artist && 'h-full place-items-center'
              }`}
          >
