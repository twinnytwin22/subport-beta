  ...
  return (
    !isLoading && (
      <div className="w-full min-h-[70vh] max-h-[80vh] mt-4 lg:mt-8 flex justify-center content-center mb-24 md:mb-0">
        <div
          className={`flex min-h-full max-h-[70vh] md:overflow-y-hidden items-center bg-white dark:bg-black w-full max-w-4xl mx-auto justify-between rounded-md border-zinc-300 dark:border-zinc-700 md:border relative`}
        >
          <div
            className={`${activeTab !== null
              ? 'hidden md:block w-48'
              : 'block max-w-sm w-full mx-auto md:w-48 h-96'
              }`}
          >
            {/* SETTINGS MENU */}
          ...
          <div
            className={`bg-white dark:bg-black relative  border-zinc-300 dark:border-zinc-800 md:border-l p-8  max-w-2xl w-full space-y-4 md:flex place-items-start h-full  overflow-y-scroll overflow-x-hidden ${activeTab !== null ? 'block' : 'hidden'
              }`}
          >
            {/* SETTINGS CONTENT */}
            <button
              className={`${activeTab !== null ? 'block md:hidden' : 'hidden'}`}
      ...
      <div className="w-full min-h-[70vh] max-h-[80vh] mt-4 lg:mt-8 flex justify-center content-center mb-24 md:mb-0">
        ...
        >
          ...
          >
            ...
            {activeTab === 'profile' && <ProfileSettings />}
            {activeTab === 'preferences' && (
              <div className="h-full">Preferences</div>
            )}
            {activeTab === 'artist-settings' && <ArtistSettings />}
            {activeTab === 'settings' && <div className="h-full">settings</div>}
          </div>
